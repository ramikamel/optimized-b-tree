...
namespace
{
    ...
    std::vector<std::string> generateWikiTitles(std::size_t count, std::mt19937_64 &rng)
    {
        ...
        for (std::size_t i = 0; i < count; ++i)
        {
            ...
            if ((i % 4) == 0)
            {
                title += "_(" + std::to_string(year_dist(rng)) + ")";
            }
    ...
    std::vector<std::string> generateIntegerStrings(std::size_t count)
    {
        ...
        for (std::size_t i = 0; i < count; ++i)
        {
            ...
            if (key.size() < 12)
            {
                key.insert(key.begin(), 12 - key.size(), '0');
            }
    ...
    std::vector<std::string> loadKeysFromFile(const std::string &file_path, std::size_t limit)
    {
        ...
        std::string line;
        while (std::getline(in, line) && keys.size() < limit)
        {
            if (!line.empty())
            {
                keys.push_back(line);
        ...
        if (keys.empty())
        {
            throw std::runtime_error("input file had no usable keys");
        }
    ...
    void runDatasetBenchmark(const std::string &dataset_name,
    ...
    for (std::size_t i = 0; i < query_count; ++i) {
      const auto result = tree.search(keys[probe_indices[i]]);
      found += result.has_value() ? 1 : 0;
    }
    std::cout << "  found=" << found << "/" << query_count << "\n"; });
    ...
    for (std::size_t i = 0; i < range_ops; ++i) {
      const auto rows = tree.rangeScan(keys[probe_indices[i]], scan_len);
      total_rows += rows.size();
    }
    std::cout << "  avg_rows=" << (range_ops ? (total_rows / range_ops) : 0) << "\n"; });

        std::cout << "  tree_size=" << tree.size() << " height=" << tree.height() << "\n";
    }
...
int main(int argc, char **argv)
{
    try
    {
        ...
        const std::vector<std::string> wiki_keys =
            (argc >= 3) ? loadKeysFromFile(argv[2], count) : generateWikiTitles(count, rng);
