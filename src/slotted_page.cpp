...
namespace abt
{
    ...
    NodeType SlottedPage::type() const
    {
        return static_cast<NodeType>(header().node_type);
    }

    std::uint16_t SlottedPage::slotCount() const
    {
    ...
    {
        return static_cast<std::size_t>(header().free_end - header().free_begin);
    }
    ...
    void SlottedPage::setPrefix(std::string_view prefix_value)
    {
        ...
        {
            throw std::logic_error("setPrefix requires an empty page");
        }
        if (sizeof(Header) + prefix_value.size() > kPageSizeBytes)
        {
            throw std::invalid_argument("prefix does not fit in page");
        }
    ...
    {
        const std::size_t need = sizeof(LeafSlot) + key_len + value_len;
        return need <= freeSpace();
    }

    bool SlottedPage::hasSpaceForInner(std::size_t key_len) const
    {
        const std::size_t need = sizeof(InnerSlot) + key_len;
        return need <= freeSpace();
    }

    bool SlottedPage::insertLeaf(std::uint16_t pos,
                                 ...
                                 std::uint16_t hint)
    {
        ...
        if (!hasSpaceForLeaf(key_suffix.size(), value_len))
        {
            return false;
        }

        const std::size_t payload_size = key_suffix.size() + value_len;
        const std::uint16_t payload_offset =
            static_cast<std::uint16_t>(h.free_end - payload_size);

        std::uint8_t *payload = payloadAt(payload_offset);
        std::memcpy(payload, key_suffix.data(), key_suffix.size());
        std::memcpy(payload + key_suffix.size(), &value, sizeof(Value));

        shiftSlotsRight(pos);
        LeafSlot slot{};
        slot.key_len = static_cast<std::uint16_t>(key_suffix.size());
        slot.payload_offset = payload_offset;
        slot.hint = hint;
        slot.value_len = static_cast<std::uint16_t>(value_len);
        setLeafSlotAt(pos, slot);

        h.slot_count = static_cast<std::uint16_t>(h.slot_count + 1);
        h.free_begin = static_cast<std::uint16_t>(h.free_begin + sizeof(LeafSlot));
        h.free_end = payload_offset;
        return true;
    }

    bool SlottedPage::insertInner(std::uint16_t pos,
                                  std::string_view key_suffix,
                                  NodeId right_child,
                                  std::uint16_t hint)
    {
        if (type() != NodeType::kInner)
        {
            return false;
        }
        ...
        if (!hasSpaceForInner(key_suffix.size()))
        {
            return false;
        }

        const std::size_t payload_size = key_suffix.size();
        const std::uint16_t payload_offset =
            static_cast<std::uint16_t>(h.free_end - payload_size);
    ...
    std::uint16_t SlottedPage::hintAt(std::uint16_t index) const
    {
        if (type() == NodeType::kLeaf)
        {
            return leafSlotAt(index).hint;
        }
        return innerSlotAt(index).hint;
    }
    ...
    void SlottedPage::setLeafValue(std::uint16_t index, Value value)
    {
        ...
        if (slot.value_len != sizeof(Value))
        {
            throw std::logic_error("value size mismatch");
        }
    ...
    {
        return innerSlotAt(index).right_child;
    }

    NodeId SlottedPage::leftChild() const
    {
        return header().left_child;
    }

    void SlottedPage::setLeftChild(NodeId id)
    {
        header().left_child = id;
    }

    NodeId SlottedPage::nextLeaf() const
    {
        return header().next_leaf;
    }

    void SlottedPage::setNextLeaf(NodeId id)
    {
        header().next_leaf = id;
    }
    ...
    std::uint8_t *SlottedPage::slotBase()
    {
        return bytes_.data() + sizeof(Header) + header().prefix_len;
    }

    const std::uint8_t *SlottedPage::slotBase() const
    {
        return bytes_.data() + sizeof(Header) + header().prefix_len;
    }

    std::uint8_t *SlottedPage::payloadAt(std::uint16_t offset)
    {
        return bytes_.data() + offset;
    }

    const std::uint8_t *SlottedPage::payloadAt(std::uint16_t offset) const
    {
        return bytes_.data() + offset;
    }

    std::size_t SlottedPage::slotSize() const
    {
        return type() == NodeType::kLeaf ? sizeof(LeafSlot) : sizeof(InnerSlot);
    }

    SlottedPage::LeafSlot SlottedPage::leafSlotAt(std::uint16_t index) const
    {
        return readSlot<LeafSlot>(index);
    }

    void SlottedPage::setLeafSlotAt(std::uint16_t index, const LeafSlot &slot)
    {
        writeSlot<LeafSlot>(index, slot);
    }

    SlottedPage::InnerSlot SlottedPage::innerSlotAt(std::uint16_t index) const
    {
        return readSlot<InnerSlot>(index);
    }

    void SlottedPage::setInnerSlotAt(std::uint16_t index, const InnerSlot &slot)
    {
        writeSlot<InnerSlot>(index, slot);
    }
